...
## Manages multiple tokens from a single node
...
func _ensure_unique_resources():
	# Make sure each token is a unique instance
	for i in range(tokens.size()):
		if tokens[i] and not tokens[i].resource_local_to_scene:
			tokens[i] = tokens[i].duplicate()

func _process(delta):
	# Constantly redraw in editor to catch property changes
...
func _get_token_at_position(pos: Vector2) -> TokenData:
	# Check in reverse order (top token first)
...
# Helper function to add tokens programmatically
